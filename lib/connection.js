...
var Connection = module.exports = function Connection(options) {
  ...
  this._btLocalBitfield = options.bitfield;
  ...
  this._btParser.on("data", function onParserData(message) {
    ...
    if (message.type === "bitfield") {
      ...
      if (!self._btLocalBitfield) {
        self._btLocalBitfield = new Bitfield(message.bitfield.length);
      }
    ...
    if (message.type === "have") {
      self._btRemoteBitfield.set(index, true);
...
Connection.prototype.bitfield = function bitfield() {
  this._btSerialiser.bitfield({
    bitfield: this._btLocalBitfield.toBuffer(),
  });
...
Connection.prototype.cancelRequest = function cancelRequest(index, begin, length) {
  this._btSerialiser.cancel({
    index: index,
    begin: begin,
    length: length,
  });
